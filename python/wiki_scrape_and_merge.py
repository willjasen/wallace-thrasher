...
def _prefer_existing_specific_speaker(existing: str, wiki_speaker: str) -> str:
    """Keep a better-cased or more specific existing speaker label."""
    existing_clean = existing.strip()
    wiki_clean = wiki_speaker.strip()
    # Capitalization alone is not a substantive wiki correction. Preserve the
    # local spelling so a label such as "EDDIE MONEY" cannot replace the
    # already curated "Eddie Money".
    if (
        existing_clean
        and wiki_clean
        and existing_clean.casefold() == wiki_clean.casefold()
    ):
...
def cmd_merge(args) -> None:
    ...
    for album, track in iter_tracks(data, args.album, args.track):
        ...
        for aln in compare["alignments"]:
            ...
            if new_spk:
                # Reapply label-quality rules at write time as protection for
                # comparisons generated by an older version of this script.
                new_spk = _prefer_existing_specific_speaker(old_spk, new_spk)
